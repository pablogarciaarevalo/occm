...
* A Professional Package that enables you to bundle Cloud Volumes ONTAP and Cloud Backup service by using an annual contract for 1, 2, or 3 years. Payment is per TiB. This option doesn’t enable you to back up on-prem data.
+
...
When you add your license to Cloud Manager, you can enter the serial number or upload the NLF license file. The steps below show how to get the NLF license file.
...
+
* If you have the serial number, select the *Enter Serial Number* option and enter the serial number.
* If you have the backup license file, select the *Upload License File* option and follow the prompts to attach the file.
+
image:screenshot_services_license_add2.png[A screenshot that shows the page to add the Backup BYOL license.]
...
. Click the chat icon in the lower-right of Cloud Manager to request an extension to your license for the particular service serial number.
+
