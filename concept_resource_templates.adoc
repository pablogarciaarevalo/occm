...
"Drift" allows Cloud Manager to monitor the parameter values used when a resource is created with the template. Then we can notify you if a parameter value is later modified and is no longer aligned with the template definition. In the future "drift" could reverse a user's change so that all resources created from a template are brought back into compliance automatically.
...
+
For example, an "action" to create a Cloud Volumes ONTAP volume or Azure NetApp Files volume.
...
. Your storage admins use the template to create resources that are optimized for the application they will be used for.
+
