...
. If you want to have more secure connectivity on public internet from your on-prem data center to the VNet, there is an option to configure an Azure Private Endpoint in the activation wizard. In this case you will need to know the Resource Group, VNet, and Subnet for this connection.

. Additionally, you use choose your own customer-managed keys for data encryption in the activation wizard instead of the default Microsoft-managed encryption keys. In this case you will need to have the Azure Subscription, Key Vault name, and the Key.
