...
The *FabricPool* license that you may have used in the past to tier on-premises ONTAP data to the cloud is being retired on August 20, 2021. It is being replaced by the *Cloud Tiering* license. Here are some thing you need to know about the new license:

* The Cloud Tiering license is a _floating_ license that you can use across multiple on-premises ONTAP clusters. This is different than in the past where you needed to purchase a FabricPool license for _every_ cluster.
* The Cloud Tiering license must be registered in Cloud Manager in the Digital Wallet. It is not applied to individual clusters.
* If you are currently using FabricPool licensing, you are not affected until your FabricPool license expires - then you'll have to start using the new floating license.
* There is a free trial so you can use the tiering service without a license for 30 days. This replaces the old feature where you could tier the first 10 TB of data for free.
* The following configurations do not currently support the new Cloud Tiering licensing. If you have one of these configurations, you must install the Cloud Tiering license on each cluster using System Manager (or the CLI) like you did with your FabricPool license - one license per system:

** ONTAP installations in the GovCloud or in Dark Sites
** ONTAP clusters in a MetroCluster configuration
** Users who are tiering data to IBM Cloud Object Storage or Alibaba Cloud Object Storage (Cloud Manager supports tiering to Amazon S3, Azure Blob storage, and Google Cloud Storage.)
