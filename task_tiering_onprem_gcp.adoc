...
* An AFF or FAS system with all-SSD aggregates that's running ONTAP 9.6 or later and has an HTTPS connection to Google Cloud Storage. link:task_discovering_ontap.html[Learn how to discover a cluster].
...
[role="quick-margin-para"]
In Cloud Manager, select an on-prem working environment, click *Enable* and follow the prompts to tier data to Google Cloud Storage.
...
NOTE: If you are planning to select a lower cost storage class where your tiered data will transition to after 30 days, you must ensure that there are no other rules applied to the selected bucket.
...
.. *Storage Class*: Select the storage class that you want to transition the tiered data to after 30 days.
+
link:reference_google_support.html[See supported storage classes^]. If you choose Standard, then the data remains in that storage class and no rules are applied.
...
. In the _Tiering Policy_ dialog, configure tiering for the selected volumes and click *Apply*.
+
link:concept_cloud_tiering.html#volume-tiering-policies[Learn more about volume tiering policies].
+
Select a tiering policy and optionally adjust the cooling days.
+
