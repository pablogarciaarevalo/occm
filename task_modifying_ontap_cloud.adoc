...
High write speed is supported with all types of single node systems. It's also supported with HA pairs in AWS and Azure when using a specific instance or VM type (link:concept_write_speed.html[click here to see the list of supported instances and VM types]). High write speed is not supported with HA pairs in GCP.

Before you change the write speed, you should link:concept_write_speed.html[understand the differences between the normal and high settings].
