...
The *Policies* tab in the Compliance Dashboard lists all the Policies available on this instance of Cloud Compliance.

image:screenshot_compliance_highlights_tab.png[A screenshot of the Policies tab in the Cloud Compliance dashboard.]
...
.. Optionally, check the box to automatically assign AIP labels to files that match the Policy parameters, and select the label. (Learn more about <<Categorizing your data using AIP labels,AIP labels>>.)
.. Click *Create Policy*.
+
...
. In the Edit Policy page, check the box if you want notification emails sent to Cloud Manager users, and choose the interval at which the email is sent (for example, *Week*).
+
