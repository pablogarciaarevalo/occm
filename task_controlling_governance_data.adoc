...
The _Top Data Repositories by Sensitivity Level_ area lists up to the top four data repositories (working environments and data sources) that contain the most sensitive items. Each bar in the chart is divided into Non-Personal data, Personal data, and Sensitive Personal data.
...
The _Open Permissions_ area shows the types of open permissions that exist for all files that are being scanned. The chart shows the following types of permissions: No Open Permissions, Open to Organization, Open to Public, and Unknown Permissions. You can hover over each section to see the total number of files in each category. Click each area to view the filtered results in the Investigation page so that you can investigate further.
