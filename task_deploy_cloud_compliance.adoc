...
Follow these steps when installing Data Sense software on a single on-prem host system.
...
Follow these steps when installing Data Sense software on multiple host systems.
...
The host requirements are the same for Manager nodes and Scanner nodes. See <<Host requirements,Host requirements>> for details.
...
. After the manager node installation completes, a dialog displays the installation command needed for the scanner nodes (starting with "sudo ./node_install.sh"). Copy the command and save it in a text file.

. On each scanner node, paste and execute the command.

.Result

The Cloud Data Sense installer installs packages, installs docker, registers the installation, installs Data Sense, and connects the manager and scanner nodes. Installation can take 10 to 20 minutes.
