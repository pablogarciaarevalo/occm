...
The on-prem installation may be a good option if you prefer to scan on-premises ONTAP working environments using a Compliance instance that's also located on premises. But this this not a requirement. The Compliance software functions exactly the same way regardless of which installation method you choose.
...
You can download and install the Compliance software on a Linux host in your network if you do not want to <<Deploying the Cloud Compliance instance in the cloud,deploy it in the cloud>>.
