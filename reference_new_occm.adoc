...
== Cloud Manager 3.9.4 (7 Mar 2021)

* <<Cloud Volumes ONTAP management enhancements>>
* <<Connector enhancements>>
* <<Additional services supported in Gov regions>>
* <<Cloud Sync enhancements>>

=== Cloud Volumes ONTAP management enhancements

This release of Cloud Manager includes enhancements to Cloud Volumes ONTAP.
...
* You can now deploy Cloud Volumes ONTAP 9.8 in the AWS Commercial Cloud Services (C2S) environment. link:??.html[Learn how to get started in C2S].

* For many releases, Cloud Manager has enabled you to encrypt Cloud Volumes ONTAP data using the AWS Key Management Service (KMS). If you choose the default Customer Master Key (CMK), data on EBS disks and data tiered to S3 are encrypted. Starting in this release, data on EBS disks and data tiered to S3 are also encrypted if you select an external key that was created outside of AWS KMS. Previously, only EBS data would be encrypted.
+
Note that you'll need to provide the Cloud Volumes ONTAP IAM role with access to use the external key. link:task_setting_up_kms.html[Learn more about setting up the AWS KMS with Cloud Volumes ONTAP].

==== Enhancement available in Azure

You can now deploy Cloud Volumes ONTAP 9.8 in the Azure Department of Defense (DoD) Impact Level 6 (IL6). link:??.html[Learn how to get started in the IL6 environment].

==== Enhancement available in Google Cloud

When you deploy a Cloud Volumes ONTAP HA pair in Google Cloud, you can now choose shared VPCs for VPC-1, VPC-2, and VPC-3. Previously, only VPC-0 could be a shared VPC.
...
The Cloud Compliance and Single File Restore services are now supported in AWS GovCloud regions and in Azure Government regions. Go to the https://cloud.netapp.com/cloud-volumes-global-regions[Global Regions Map on NetApp Cloud Central^] to view the list of supported Gov regions in AWS and Azure.

NOTE: These services are not supported in _secret_ Government environments.
