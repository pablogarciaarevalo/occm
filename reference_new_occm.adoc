...
+
link:??.html[Learn how to get started in C2S].

* Cloud Manager has always enabled you to encrypt Cloud Volumes ONTAP data using the AWS Key Management Service (KMS). If you choose the default Customer Master Key (CMK), data on EBS disks and data tiered to S3 are encrypted. Starting in this release, data on EBS disks and data tiered to S3 are encrypted if you select an _external_ key that was created outside of AWS KMS. Previously, only EBS data would be encrypted.
+
Note that you'll need to provide the Cloud Volumes ONTAP IAM role with access to use the external key.
+
...
link:??.html[Learn how to get started in the IL6 environment].
