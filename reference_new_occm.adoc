...
* Cloud Manager has always enabled you to encrypt Cloud Volumes ONTAP data using the AWS Key Management Service (KMS). If you choose the default Customer Master Key (CMK), data on EBS disks and data tiered to S3 are encrypted. Starting with Cloud Volumes ONTAP 9.9.0, data on EBS disks and data tiered to S3 are encrypted if you select an _external_ key that was created outside of the AWS KMS. Previously, only EBS data would be encrypted.
+
