...
* Cloud Volumes ONTAP now supports _General Purpose SSD (gp3)_ disks in AWS. gp3 disks are the lowest-cost SSDs that balance cost and performance for a broad range of workloads.
+
...
* Cloud Volumes ONTAP no longer supports Cold HDD (sc1) disks in AWS.
...
+
link:screenshot_sync_warning.gif["A screenshot that shows a warning when viewing a data broker: This Data Broker isn't running the latest software release (1.8). If you experience performance problems please contact us."]
...
* You can view a list of link:task_controlling_private_data.html#viewing-file-metadata-and-permissions[all users or groups who have access to a file^].
