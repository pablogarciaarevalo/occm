...
* Cloud Volumes ONTAP now supports _General Purpose SSD (gp3)_ disks.
+
gp3 disks are the lowest-cost SSDs that balance cost and performance for a broad range of workloads.
+
...
+
link:screenshot_sync_warning.gif[A screenshot that shows a warning when viewing a data broker on the Dashboard.]
