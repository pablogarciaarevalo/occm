...
== Cloud Manager 3.9.8 (6 July 2021)
...
+
** *Capacity-based BYOL*: A capacity-based license purchased from NetApp enables you to pay for Cloud Volumes ONTAP by capacity. The license is associated with your NetApp account and supports multiple Cloud Volumes ONTAP systems. Each individual system can have up to 2 PB of capacity using disks and data tiering to object storage.
** *Freemium*: The Freemium charging method enables you to use all Cloud Volumes ONTAP features free of charge from NetApp (cloud provider charges still apply). You're limited to 500 GB of allocated capacity per system and there's no support contract.
+
Here's an example of the charging methods that you can choose from deploying a new Cloud Volumes ONTAP system in Azure:
+
...
Starting with the 9.9.1 release, Cloud Volumes ONTAP now supports the m5dn.24xlarge instance type with PAYGO Premium, BYOL, and Freemium.
...
* When creating a storage account for Cloud Volumes ONTAP, Cloud Manager now disables *Blob public access* on the storage account.

* By default, Cloud Manager now enables an Azure Private Link connection on the boot diagnostics storage account for Cloud Volumes ONTAP.
+
...
* Cloud Volumes ONTAP now supports Balanced persistent disks (pd-balanced) starting with the 9.9.1 release.
+
...
NetApp Support Site (NSS) accounts are now managed in the Support tab, rather than from the Settings menu. This change makes it easier to find and manage all support-related information from a single location.
...
The Digital Wallet page is now located in its own tab under the Resources section.
