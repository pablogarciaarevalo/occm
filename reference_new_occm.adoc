...
* <<NetApp's Identity as a Service>>

=== Cloud Volumes ONTAP enhancements

* You can now pay for Cloud Volumes ONTAP and Cloud Backup Service by using an annual contract from the AWS Marketplace that charges by capacity. If you choose this payment option, you can provision up to 2 PB per Cloud Volumes ONTAP system (single node or HA).
+
Be aware that disk limits can prevent you from reaching the capacity limit by using disks alone. You can go beyond the disk limit by link:concept_data_tiering.html[tiering inactive data to object storage].
+
https://aws.amazon.com/marketplace/pp/prodview-q7dg6zwszplri[View pricing details for the Professional Edition on the AWS Marketplace page^].
...
+
link:task_tiering.html[Learn how to adjust the minimum cooling period.]
...
+
Note that a Connector requires a new permission to delete Azure snapshots:
+
[source,json]
"Microsoft.Compute/snapshots/delete"
+
https://mysupport.netapp.com/site/info/cloud-manager-policies[View the latest Cloud Manager policy for Azure^].

* In AWS, Cloud Manager now adds tags to EBS volumes when creating a new Cloud Volumes ONTAP working environment. The tags were previously created after Cloud Volumes ONTAP was deployed.
+
This change can help if your organization uses service control policies (SCPs) to manage permissions.

=== Digital Wallet

A new *Digital Wallet* feature enables you to more easily view and manage your Cloud Volumes ONTAP licenses and Cloud Backup licenses from a single location. link:task_managing_licenses.html#[Learn more about Digital Wallet].

image:screenshot_digital_wallet.gif[A screenshot of the Digital Wallet page that shows Cloud Volumes ONTAP licenses.]

=== Cloud Backup enhancements

* Cloud Backup now allows you to create a backup using a different account/subscription than that used for your Cloud Volumes ONTAP system.
+
You can also create backup files in a different region than the one in which your Cloud Volumes ONTAP system is deployed.

* When configuring backups on Azure you can now select advanced networking options so that you can use a different Azure Virtual Network (VNet) and Subnet.
...
** You can choose the number of days when tiered data will move to a more cost-effective tier.
** Azure Blob now supports the _Cool_ access tier.
** Google Cloud Storage supports moving tiered data to multiple storage tiers over time. For example, you can move tiered data from the _Standard_ class to the _Nearline_ class after 30 days, and then to the _Coldline_ class after 60 days, and then to the _Archive_ class after 270 days.
...
+
The report provides overview pages that summarize all your working environments and data sources, and then provides a breakdown for each working environment.
...
+
This is helpful to identify areas where you can save storage space, or identify files that have specific permissions or sensitive information that you do not want duplicated across your storage.

* You can add a custom Status to files that Data Sense is scanning. The Status is not added to the file in the same way as AIP Labels are added. The Status is just seen by Cloud Manager users so you can indicate if a file needs to be deleted, or checked for some reason.
+
...
* New functionality called "drift" has been added as an option when you create your templates.
+
...
* Now you can run a template from the Template Dashboard instead of having to open a working environment and launch the template there.

=== NetApp's Identity as a Service

NetApp now uses Microsoft Azure Active Directory as the identity provider for authentication services specific to support and licensing.

When you add a new NetApp Support Site (NSS) account to Cloud Manager, you'll be redirected to a Microsoft login page where you will need to provide your NetApp Support Site (NSS) registered email and password to perform the authentication process. This action enables Cloud Manager to use your NSS account.

At this time, there's no impact to NSS accounts that you have already added to Cloud Manager.

link:https://kb.netapp.com/Advice_and_Troubleshooting/Miscellaneous/FAQs_for_NetApp_Adoption_of_MS_Azure_AD_B2B_for_Login[FAQs for NetApp Adoption of MS Azure AD B2B for Login^]
