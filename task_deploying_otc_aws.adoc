...
| [[subscribe]]Edit Credentials | Choose the AWS credentials and marketplace subscription to use with this Cloud Volumes ONTAP system.
...
. *License and NSS Account*: Specify which licensing option would you like to use with this system, and then specify a NetApp Support Site account.
+
To understand how BYOL licenses work, see link:concept_licensing.html[Licensing].
+
A NetApp Support Site Account is required for BYOL systems. link:task_adding_nss_accounts.html[Learn how to add NetApp Support Site accounts].

. *Cloud Volumes ONTAP Configuration* (annual contract only): Review the default configuration and click *Continue* or click *Change Configuration* to select your own configuration.
+
