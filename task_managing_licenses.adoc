...
* Pay as you go (PAYGO) licenses, which enables you to pay by the hour through a marketplace subscription from your cloud provider.
* Node-based licenses, which are the previous generation BYOL for Cloud Volumes ONTAP. This includes Eval licenses that you can later convert to the node-based BYOL licensing model.
