...
* *Get output value from previous action* - The value Cloud Manager generates after creating a resource from a previous template action.
* *Get value from existing resource* - The value from a resource that already exists - for example an existing volume.

Operator::
The operator for the comparison. The options are:
* *Equals*
* *Greater Than*
* *Less Than*
