...
Disks::
Customer data resides on Premium Storage page blobs. Each node has access to the other node's storage. Additional storage is also required for link:reference_default_configs.html#boot-and-root-data-for-cloud-volumes-ontap[boot, root, and core data].
