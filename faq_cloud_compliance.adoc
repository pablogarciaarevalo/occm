...
Cloud Compliance supports scanning of unstructured data over NFS and CIFS protocols that are managed by Cloud Volumes ONTAP, Azure NetApp Files, on-prem ONTAP systems, and non-NetApp file shares. Cloud Compliance can also scan data stored on Amazon S3 buckets and generic S3 object storage.
