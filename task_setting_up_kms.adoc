...
. If you are using an _external_ key that was created from outside of the AWS KMS, modify the key policy for the CMK by adding the IAM role that provides permissions to Cloud Volumes ONTAP as a key user.
+
...
+
You'll need to perform this step _after_ you deploy Cloud Volumes ONTAP because the IAM role is created when you create a working environment. (Of course, you do have the option to select an existing Cloud Volumes ONTAP role, so you can do this before.)
