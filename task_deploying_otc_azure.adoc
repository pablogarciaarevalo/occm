...
| Resource Group | Create a new resource group for Cloud Volumes ONTAP or use an existing resource group.

The best practice is to use a new, dedicated resource group for Cloud Volumes ONTAP. While it is possible to deploy Cloud Volumes ONTAP in an existing, shared resource group, it's not recommended due to the risk of data loss. See the warning above for more details.
...
. *Charging Methods and NSS Account*: Specify whether you want to use pay-as-you-go or BYOL, and then specify a NetApp Support Site account.
+
