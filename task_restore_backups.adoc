...
NOTE: Restoring individual files from a backup file uses a separate Restore instance/virtual machine. See the Requirements section for link:task_backup_to_s3.html#requirements[AWS] or link:task_backup_to_azure.html#requirements[Azure] to make sure your environment has been set up correctly.
