...
You access the Restore Dashboard by clicking the *Backup & Restore* tab from the top of Cloud Manager, or you can click the *Enable* button for the Restore service from the Services panel.
...
* When restoring from StorageGRID, enter the IPspace in the ONTAP cluster where the destination volumes reside, and the Access Key and Secret Key needed to access the object storage.
