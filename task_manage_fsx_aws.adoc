...
keywords: amazon fsx for ontap, fsx, volumes, create volumes, delete volumes, access key, secret key, region, working environment, aws transit gateway, cmk
summary: Cloud Manager enables you to create volumes in Amazon FSx for ONTAP.
---
...
The first step is to create an FSx for ONTAP working environment. If you already created an FSx for ONTAP file system in the AWS Management Console, you can link:task_manage_fsx_aws#discover_existing_fsx_ontap_file_system[discover it using Cloud Manager].
...
. Leave _CIDR Range_ empty and click *Next* to automatically set an available range. Optionally, you can use AWS Transit Gateway to manually configure a range.
+
...
If you created an FSx for ONTAP file system using the AWS Management Console or if you want to restore a working environment you removed from the Cloud Manager, you can discover it using Cloud Manager.

.Steps

. In Cloud Manager, click *Add Working Environment*, select *Amazon Web Services*, and click *Next*.
