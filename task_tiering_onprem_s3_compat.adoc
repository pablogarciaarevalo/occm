...
Licensing::
A FabricPool BYOL license is required on the ONTAP cluster when tiering data to S3-compatible object storage. If you don't currently have a FabricPool license, link:task_licensing_cloud_tiering.html#adding-a-tiering-license-to-ontap[see how to add a tiering license now].
...
. *Choose your provider*: Select *S3 Compatible*.
...
. On the _Tier Volumes_ page, select the volumes that you want to configure tiering for and click *Continue* to launch the Tiering Policy page:
