...
+
A note about SMB servers: if you have a new domain, then you'll need to specify it when you update the credentials. If you entered a domain when you created the sync relationship, but you don't enter a new domain when you update the credentials, then Cloud Sync will keep using the original domain that you provided.
