...
| Volume Name | Enter the volume name. The template designer may allow you to enter any volume name here. However, there may be instructions within the field that you need to use a certain prefix or suffix, that it _contains_ certain characters within the name, or that it follows rules from a regular expression (regex).

For example, "db" could be a required prefix, suffix, or contains; meaning your volume should be named "db_vol1", "vol1_db, "vol_db_1", etc..

| Volume Size | Specify the size of the volume in GB.
...
. *Disk Type:* Choose the cloud storage provider (if not preselected), the type of disk, and the capacity tier (optional) that will be used for the volume storage.
...
| Export Policy | Enter an export policy to define the clients in the subnet that can access the volume.
...
| Share Name | The template designer may have allowed you to enter any share name here, but there may be instructions within the field that you need to use a certain prefix or suffix, that it _contains_ certain characters within the name, or that it follows rules from a regular expression (regex).
...
. If there are no other Actions that you need to define, click *Create Volume*.
+
