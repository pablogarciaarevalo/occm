...
. In the Template dashboard, select the template that you want to use to create the volume and click *Next*.
+
...
Cloud Volumes ONTAP provisions the volume. Additionally, if any secondary action is implemented in the template, for example, enabling Cloud Backup on the volume, that action is also performed.
