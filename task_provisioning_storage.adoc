...
A Cloud Manager feature called "templates" enables you to create volumes that are optimized for the workload requirements for certain applications; such as databases or streaming services. If your organization has created some volume templates, you can follow these same steps. Just be aware that the template may pre-define some fields depending on the workloads that will run on the volume.
