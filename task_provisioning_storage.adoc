...
If your organization has created volume templates so you can deploy volumes that are optimized for the workload requirements for certain applications, these are the steps you should follow.

The template should make your job easier because certain volume parameters will already be defined in the template, such as disk type, size, protocol, snapshot policy, cloud provider, and more. When a parameter is already defined, you can just skip to the next volume parameter.

.Steps

. On the Canvas page, double-click the name of the Cloud Volumes ONTAP system on which you want to provision a volume.

. Click *Add Volume* > *New volume from template*.
+
image:screenshot_template_dashboard.png[A screenshot of the available application templates.]

. Select the template that you want to use to create the volumes and click *Next*.
+
...
| Volume Name | Enter the volume name. The template designer may have allowed you to enter any volume name here, but there may be instructions within the field that you need to use a certain prefix or suffix, that it _contains_ certain characters within the name, or that it follows rules from a regular expression (regex).
...
. *Disk Type:* Choose the cloud storage provider and the type of disk that will be used for the volume storage.

. *Protocol:* Select *NFS* or *CIFS* and then the provide the protocol details.
+
[cols=2*,options="header",cols="25,75"]
...
| Share Name (CIFS only) | The template designer may have allowed you to enter any share name here, but there may be instructions within the field that you need to use a certain prefix or suffix, that it _contains_ certain characters within the name, or that it follows rules from a regular expression (regex).
...
. In the _Add Actions_ page, if required, enable Backups and set the policy to create backups with a certain retention value.
