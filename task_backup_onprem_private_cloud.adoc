...
StorageGRID must meet the following requirements. See the https://docs.netapp.com/sgws-114/index.jsp^[StorageGRID documentation] for complete information about StorageGRID].
...
.. The Access Key and the Secret Key used to store the backups.
...
. Click *Next* and Cloud Backup starts taking the initial backups of each selected volume and the Backup Dashboard is displayed so you can monitor the state of the backups.
