...
* You need to have an account that has an access key and the required permissions so the ONTAP cluster can back up and restore data.
...
+
* An inbound connection is required from the Connector, which can reside in a Google Cloud Platform VPC.
