...
. Select the AWS Account and the region where you want to create the backups, and click *Next*.
// This can be a different AWS Account and region than where the Cloud Volumes ONTAP system resides.
+
image:screenshot_backup_provider_settings_aws.png[A screenshot that shows the cloud provider details when backing up volumes from an ONTAP cluster to an AWS S3]
