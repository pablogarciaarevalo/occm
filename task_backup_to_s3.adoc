...
+
This can be a different AWS Account and region than where the Cloud Volumes ONTAP system resides. If you want to use a different AWS account for your backups, you must link:reference_backup_multi_account_aws.html[log in to the AWS portal and link the two accounts].
+
image:screenshot_backup_provider_settings_aws.png[A screenshot that shows the cloud provider details when backing up volumes from an ONTAP cluster to an AWS S3]
