...
== Configure the data broker to copy object storage metadata

By default, Cloud Sync copies object storage metadata for link:faq_sync.html#object-storage-metadata[several types of sync relationships]. But you need to configure the data broker if you want to copy object storage metadata between the following types of sync relationships:
...
.Steps for Amazon S3 to Amazon S3

. Log in to the data broker host.

.	Modify local.json as follows:
+
[source,json]
{
  "protocols": {
...
.Steps for Google Cloud to StorageGRID, IBM, or Amazon S3
