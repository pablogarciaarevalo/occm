...
+
TIP: If you are using a shared VPC to deploy resources into a service project, then the Connector service account will also require the _compute.networkUser_ role within the host project. https://cloud.google.com/iam/docs/job-functions/networking[This role gives the Connector VM permission to use the shared VPC^].
