...
TIP: If you are using a shared VPC to deploy resources into a service project, then the following permissions table applies:
...
| Cloud Manager Service Account | Custom | Service Project |
+
* https://occm-sample-policies.s3.amazonaws.com/Policy_for_Cloud_Manager_3.9.0_GCP.yaml[Permissions Here^] 
...
+
Note: deploymentmanager.editor is only required if you are not passing firewall rules to the deployment and are choosing to let Cloud Manager create them for you. | Deploying and maintaining Cloud Volumes ONTAP and services in service project
| Cloud Volumes ONTAP Service Account | Custom | Service Project |
+
* storage.Admin
* Member: Cloud Manager Service Account as service account user
+
| N/A | (Optional) For Cloud Tiering and Cloud Backup
| Google APIs Service Agent | GCP | Service Project |
+
* (Default) Editor
+
|
...
| Google Compute Engine default service account | GCP | Service Project |
+
* (Default) Editor
+
|
