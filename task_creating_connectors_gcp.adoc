...
If you are using a shared VPC to deploy resources into a service project, then the following permissions table applies. This table is for reference and your environment should reflect the permissions table when IAM configuration is complete:
...
* https://occm-sample-policies.s3.amazonaws.com/Policy_for_Cloud_Manager_3.9.0_GCP.yaml[Permissions here in .yaml file^]
a| 
* compute.networkUser
* deploymentmanager.editor
| Deploying and maintaining Cloud Volumes ONTAP and services in service project
| Cloud Volumes ONTAP Service Account | Custom | Service Project a|
* storage.admin
* member: Cloud Manager Service Account as serviceAccount.user
| N/A | (Optional) For Cloud Tiering and Cloud Backup
| Google APIs Service Agent | GCP | Service Project a|
* (Default) Editor
a|
* compute.networkUser
| Interacts with GCP APIs on behalf of Deployment. Allows Cloud Manager to use shared network
| Google Compute Engine default service account | GCP | Service Project a|
* (Default) Editor
a|
* compute.networkUser
| Deploys GCP instances and compute infrastructure on behalf of Deployment. Allows Cloud Manager to use shared network

|===

NOTE: deploymentmanager.editor is only required at the host project if you are not passing firewall rules to the deployment and are choosing to let Cloud Manager create them for you.

NOTE: firewall.create and firewall.delete are only required if you are not passing firewall rules to the deployment and are choosing to let Cloud Manager create them for you.

NOTE: For tiering, the tiering service account must have the serviceAccount.user role on the service account, not just at the project. Currently if you assign serviceAccount.user at the project level, the permissions do not show when you query the service account with getIAMPolicy.
