...
Cloud Data Sense enables you to run a general "mapping" scan on selected working environments and data sources. Mapping provides only a high-level overview of your data, whereas Classification provides or deep-level scanning of your data. Mapping can be done on your data sources very quickly because it does not access files to see the data inside.

Some users like this functionality because they want to quickly scan their data to identify the areas that require more details - and then they can enable classification only on those required data sources.
