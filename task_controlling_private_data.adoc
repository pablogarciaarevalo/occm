...
=== Viewing open permissions for files

To view a list of all users or groups who have access to a file, click *View all Permissions*. This button is available only for files in CIFS shares and OneDrive accounts.
...
If there are duplicates of a certain file, this information appears next to the _Duplicates_ field. This is useful if you want to identify areas where you can save storage space. It can also be helpful to make sure certain files that have specific permissions or sensitive information are not unnecessarily duplicated in your storage systems.

Only files that are in the formats described in link:reference_private_data_categories.html#types-of-files[Types of files] can be identified as duplicate.
...
You can download the list of duplicate files and send it to your storage admin so they can decide which files, if any, can be deleted.  Of you can link:task_managing_highlights.html#deleting-source-files[delete the file^] yourself if you are confident that this version of the file is not needed.

TIP: You can use the "hash" value provided in this page and enter it directly in the Filters page to search for a specific duplicate file at any time - or to be used in a Policy.
