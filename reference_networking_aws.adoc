...
+
A storage VM management LIF is required for management tools like SnapCenter.

==== HA pairs

Cloud Manager allocates 15 IP addresses to HA pairs deployed in a single Availability Zone (AZ) and 17 IP addresses to an HA pair deployed in multiple AZs.

The following table provides details about these LIFs.
...
| Floating IP LIF (multi-AZs only) | eth0 | node 1 and node 2 | An iSCSI LIF that manages the migration of floating IPs when Cloud Volumes ONTAP is deployed in multiple Availability Zones.
...
| Mediator | eth0	| Mediator |
|===

The following image shows these interfaces on a Cloud Volumes ONTAP HA pair.
...
Note that Cloud Manager doesn't create a storage VM management LIF on HA pairs in a single AZ. You can choose whether to create a management LIF on HA pairs in multiple AZs.
