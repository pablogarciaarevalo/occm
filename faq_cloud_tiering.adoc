...
* When tiering data to Blob storage, a Connector can reside in an Azure VNet or on your premises.
* When tiering data to Google Cloud Storage, a Connector must reside in a Google Cloud Platform VPC.
* When tiering data to StorageGRID or other S3-Comatible storage providers, a Connector must reside on an on-premises Linux host.
