...
Cloud Backup enables you to delete a single backup file or _all_ backups of a specific volume. You might want to delete all backups if you no longer need the backups or if you deleted the source volume and want to remove all backups.

Note that deleting all backups does not disable further backups of this volume. If you want to stop creating backups of the volume, you can disable backups <<Starting and stopping backups of volumes,as described here>>.
