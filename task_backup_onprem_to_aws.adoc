...
[role="quick-margin-para"]
Select the IPspace in the ONTAP cluster where the volumes reside. You can also choose to use an existing private VPC endpoint.
...
. If you want to have more secure connectivity over the public internet from your on-prem data center to the VPC, there is an option to select a private VPC endpoint in the activation wizard. In this case you'll need to have created an endpoint service configuration using the Amazon VPC console or the command line. https://docs.aws.amazon.com/vpc/latest/privatelink/endpoint-service.html[See details about using a Private Endpoint^].
...
.. Whether you'll use a private VPC endpoint. https://docs.aws.amazon.com/vpc/latest/privatelink/endpoint-service.html[See details about using a Private Endpoint].
+
image:screenshot_backup_onprem_aws_networking.png[A screenshot that shows the networking details when backing up volumes from an ONTAP system to AWS S3.]
