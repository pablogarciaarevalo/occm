...
== Create sync relationships for Cloud Volumes ONTAP, Azure NetApp Files, Amazon FSx for ONTAP, or on-prem clusters

If you want to create sync relationships for Cloud Volumes ONTAP, Azure NetApp Files, Amazon FSx for ONTAP, or an on-prem ONTAP cluster, then you first need to create or discover the working environment.
...
* link:task_manage_fsx.html[Setting up and discovering Amazon FSx for ONTAP]
...
Use these steps to sync data to or from a supported storage type other than Cloud Volumes ONTAP, Azure NetApp Files, Amazon FSx for ONTAP, or on-prem ONTAP clusters. The steps below provide an example that shows how to set up a sync relationship from an NFS server to an S3 bucket.
